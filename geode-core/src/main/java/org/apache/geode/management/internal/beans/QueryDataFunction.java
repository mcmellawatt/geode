      ...
      if (region.getAttributes().getDataPolicy() == DataPolicy.NORMAL) {
        ...
        if (parRegion != null && showMember) {
          if (parRegion.isDataStore()) {
            ...
            ExecutionContext executionContext = new QueryExecutionContext(null, cache, query);
            results = lds.executeQuery(query, null, localPrimaryBucketSet, executionContext);
          }
        } else {
