...
import static org.apache.geode.cache.wan.GatewaySender.CONNECTION_RETRY_INTERVAL;
...
public class NewWanAuthenticationDUnitTest extends WANTestBase {
  ...
  private static final String securityJsonResource = "org/apache/geode/security/templates/security.json";
  ...
  public void testWanAuthValidCredentials() {
    ...
    // this tests verifies that even though the receiver has invalid credentials, the sender can still send data to
  ...
  public void testWanAuthInvalidCredentials() {
    ...
    receiver.invoke(() -> {
      createSecuredReceiver(nyPort, regionName, receiverSecurityPropsWithIncorrectSenderCreds, receiverJavaProps);
    });
  ...
  public void testWanSecurityManagerWithInvalidThenValidCredentials() {
    ...
    final String securityJsonResource = "org/apache/geode/internal/cache/wan/misc/NewWanAuthenticationDUnitTest.testWanSecurityManagerWithInvalidCredentials.security.json";
    final Properties receiverSecurityPropsWithCorrectSenderCreds = buildSecurityProperties(securityJsonResource);
    ...
    receiver.invoke(() -> {
      createSecuredReceiver(nyPort, regionName, receiverSecurityPropsWithIncorrectSenderCreds, null);
    });
  ...
  public void testWanSecurityManagerWithValidThenInvalidThenValidCredentials() {
    ...
    /*
     * Wait long enough that the ack reader thread has a chance to attempt reconnection.
     * We want to ensure that an AuthenticationFailedException does not result in the event
     * processor being stopped.
     *
     * Empirical evidence showed that waiting twice the connection retry interval was
     * sufficient. A few problems with this approach:
     *
     * 1. There is a race between the ack reader thread and the dispatcher thread, to establish
     *    the connection. We are relying on indeterminism to ensure that over (calendar) time,
     *    each of those will get a turn winning.
     * 2. pauses (e.g. GC) can mean that even though we waited, neither thread has actually
     *    tried to authenticate yet.
     *
     * The alternative to this sleep was to test this behavior with a unit test (of the
     * GatewaySenderEventRemoteDispatcher.AckReaderThread) instead. Such a unit test, however,
     * wouldn't find bugs in the dispatcher side, so we're keeping the sleep and the DUnit test.
     */
    try {
      Thread.sleep(CONNECTION_RETRY_INTERVAL * 2);
    } catch (InterruptedException e) {
      e.printStackTrace();
    }

    /*
     * This is a key assertion--that bad credentials don't stop the event processor. If this
     * assertion fails, the test fails fast.  Otherwise, we'll test that communication can be fully
     * restored once valid credentials are used.
     */
    sender.invoke(() -> assertFalse(((AbstractGatewaySender) cache.getGatewaySender(senderId)).getEventProcessor().isStopped()));
